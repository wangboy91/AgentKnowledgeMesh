...
**澄清（非固定长度切片）**：`chunk_size=512` 是**单块 token 预算上限**而非固定切片长度。实际以语义边界优先：标题层级变化强制换新块；代码块/表格原子保留不切断；同一标题下段落累积到预算才换块；仅当单个段落块超过预算时才按断句点续切并保留句级重叠。因此各 chunk 天然不等长。

**局限**：标题只以文本形式前置，无独立权重；标题层级过碎时会产生大量小 chunk（向量信息量低、检索噪声大），缺少最小块合并；无标题的长文档退化为句子边界切分，未做基于 topic/embedding 相似度的语义分段。
...
- **分块语义化增强**（中风险，评估驱动）：
  - 小标题块合并：低于 token 下限的相邻标题块合并，减少碎片化向量。
  - 无结构文档主题分段：对无标题长文按句子 embedding 相邻相似度聚段（jina-text-segments 思路），成本高按需引入。
- 查询扩展/改写：当前无。可后置（P3）。
